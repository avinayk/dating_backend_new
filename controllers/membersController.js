...
exports.paymentdatasave = async (req, res) => {
  const { detail, user_id, paymentdetail } = req.body;
  ...
  try {
    ...
    const insertQuery = `
        INSERT INTO allmembership (user_id, start_date, end_date, days, plan, amount, payment_id, currency, livemode, status, date)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);
      `;
